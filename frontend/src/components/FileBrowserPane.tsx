  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 border-b border-gray-200">
        <span className="text-navy-600">{meta.icon}</span>
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 border-b border-gray-100 bg-white">
        <button onClick={navigateUp} className="p-1.5 rounded hover:bg-gray-100 text-gray-500 hover:text-navy-600" title="Go up" aria-label="Go up">
        ...
        {isBackend(platform) && (
          <button
            ...
            className="p-1.5 rounded hover:bg-gray-100 text-gray-500 hover:text-navy-600"
            title="New folder"
          ...
          {editingAddress ? (
            <form onSubmit={handleAddressSubmit} className="flex">
              <input
                ...
                className="w-full px-2 py-1 text-xs border border-navy-600 rounded focus:outline-none"
              />
            </form>
          ) : (
            <button
      ...
      {showNewFolder && (
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100 bg-navy-50">
          ...
          />
          <button onClick={handleCreateFolder} className="text-xs text-navy-600 font-medium">Create</button>
          ...
          {platformDatasets.map((ds: any) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-800 truncate group-hover:text-navy-600">{ds.name}</p>
                {ds.description && <p className="text-[10px] text-gray-400 truncate">{ds.description}</p>}
              </div>
              <ChevronRight className="h-3.5 w-3.5 text-gray-300 group-hover:text-navy-600" />
            </button>
          ))}
        </div>
      )}
              ...
              >
                ...
                <span className="w-6 flex-shrink-0">
                  <button onClick={e => { e.stopPropagation(); toggleSelect(entry); }}>
                    {selected
                      ? <CheckSquare className="h-3.5 w-3.5 text-navy-600" />
                ...
                <span
                  className={`flex-1 truncate ${
                    isDir ? 'font-medium text-gray-800 hover:text-navy-600 hover:underline' :
                ...
                {isDir && selected && <span className="text-[9px] bg-blue-100 text-navy-600 px-1 py-0.5 rounded mr-1 font-medium">FOLDER</span>}
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-t border-gray-200 bg-gray-50 text-[10px] text-gray-500">
        ...
        <div className="flex items-center gap-2">
          ...
          {entries.length > 0 && (
            <button onClick={selectAll} className="text-navy-600 hover:underline">
