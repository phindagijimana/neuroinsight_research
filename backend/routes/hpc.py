...
def hpc_health():
    ...
    try:
        ...
        logger.debug("SLURM health check fell through: %s", e)
...
def get_remote_system_info():
    ...
    if backend.backend_type == "remote_docker":
        ...
        try:
            ...
            logger.debug("Could not fetch remote system info: %s", e)
        return info
    else:
    ...
    for p in partitions:
        ...
        if timelimit and timelimit != "infinite":
            try:
                ...
                logger.debug("Could not parse timelimit '%s': %s", timelimit, e)
...
def browse_remote(path: str = "~"):
    ...
    if path.startswith("~"):
        try:
            ...
            logger.debug("Could not resolve home directory for '~' expansion: %s", e)
