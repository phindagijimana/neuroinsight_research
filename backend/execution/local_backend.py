...
class LocalDockerBackend(ExecutionBackend):
        ...
        def _execute():
            try:
                ...
                _sync_job_to_db(job_id, "running", backend_job_id=container.id[:12],
                               progress=3, current_phase="Starting container")

                # Stream logs and update progress (mirrors Celery task logic)
                ...
                for log_chunk in container.logs(stream=True, follow=True):
                    ...
                    for marker, pct, label in milestones:
                        if pct > current_progress and marker in text:
                            current_progress = pct
                            _sync_job_to_db(job_id, "running",
                                            progress=current_progress,
                                            current_phase=label)
                            with self._lock:
                                self._jobs[job_id]["progress"] = current_progress
                                self._jobs[job_id]["current_phase"] = label
                            break
                ...
                # Save full log
                try:
                    (output_dir / "logs" / "stdout.log").write_text(log_buffer)
                except Exception as e:
                    logger.debug("Could not save logs for job %s: %s", job_id[:8], e)
                ...
                if exit_code == 0:
                    ...
                    try:
                        ...
                        logger.warning("MinIO upload failed for job %s: %s", job_id[:8], e)
                    try:
                        _extract_bundle(job_id, spec_dict, output_dir, client)
                    except Exception as e:
                        logger.warning("Bundle extraction failed for job %s: %s", job_id[:8], e)
                else:
                    with self._lock:
                ...
                try:
                    ...
                    logger.debug("Could not remove container for job %s: %s", job_id[:8], e)
                ...
                try:
                    ...
                    logger.error("Failed to sync failed job %s to DB: %s", job_id[:8], e2)
    ...
    def get_job_status(self, job_id: str) -> JobStatus:
        ...
        try:
            ...
            logger.debug("DB lookup failed for job %s status: %s", job_id[:8], e)
    ...
    def get_job_info(self, job_id: str) -> JobInfo:
        ...
        try:
            ...
            logger.debug("DB lookup failed for job %s info: %s", job_id[:8], e)
    ...
    def _stop_container_by_label(self, job_id: str) -> bool:
        """Find and stop a Docker container by its neuroinsight.job_id label.

        Survives backend restarts where the in-memory cache is lost.
        """
        try:
            import docker
            client = docker.from_env()
            containers = client.containers.list(
                filters={"label": f"neuroinsight.job_id={job_id}"}
            )
            for c in containers:
                c.stop(timeout=10)
                logger.info(f"Stopped container {c.short_id} for job {job_id[:8]} (found by label)")
            return len(containers) > 0
        except Exception as e:
            logger.warning(f"Label-based container stop failed for job {job_id[:8]}: {e}")
            return False
    ...
    def cancel_job(self, job_id: str) -> bool:
        ...
        if not job_info:
            # No in-memory cache (e.g. after restart) -- use DB + label lookup
            try:
                from backend.core.database import get_db_context
                from backend.models.job import Job
                ...
                with get_db_context() as db:
                    ...
                    if job.status in ("completed", "failed", "cancelled"):
                        return False
                    self._stop_container_by_label(job_id)
                    job.mark_cancelled()
                    db.commit()
                    return True
        ...
        # Try to stop Docker container (from in-memory cache first, label fallback)
        ...
        try:
            ...
            logger.warning("Failed to sync cancelled status for job %s: %s", job_id[:8], e)
        ...
        if not stdout:
            ...
            if container_id:
                try:
                    ...
                    logger.debug("Could not read container logs for job %s: %s", job_id[:8], e)
