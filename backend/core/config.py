...
class Settings(BaseSettings):
    ...
    cors_origins: str = Field(
        default="http://localhost:3000,http://localhost:3051,http://localhost:5173",
    ...
    hpc_fs_license_path: Optional[str] = Field(
        default=None,
        description="Path to FreeSurfer license.txt ON the HPC filesystem (not local). "
                    "Falls back to fs_license_path if unset.",
    )
    hpc_meld_license_path: Optional[str] = Field(
        default=None,
    ...
    pennsieve_api_url: str = Field(
        default="https://api.pennsieve.io",
        description="Pennsieve API base URL",
    )
