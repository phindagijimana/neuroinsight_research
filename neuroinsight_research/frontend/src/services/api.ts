...
export const apiService = {
   ...
   * Submit a plugin job
   */
  async submitPluginJob(pluginId: string, inputFiles: string[], parameters: Record<string, any> = {}, customResources?: Record<string, any>, dataSourcePlatform?: string, dataSourceDatasetId?: string): Promise<any> {
    const response = await api.post(`/api/plugins/${pluginId}/submit`, {
      input_files: inputFiles,
      parameters,
      custom_resources: customResources,
      data_source_platform: dataSourcePlatform || null,
      data_source_dataset_id: dataSourceDatasetId || null,
    });
    return response.data;
  },
   ...
   */
  async submitWorkflowJob(workflowId: string, inputFiles: string[], parameters: Record<string, any> = {}, customResources?: Record<string, any>, dataSourcePlatform?: string, dataSourceDatasetId?: string): Promise<any> {
  ...
  async platformDisconnect(platform: string): Promise<void> {
    await api.post(`/api/platforms/${platform}/disconnect`);
  },

  /** Check platform connection status. */
  async platformStatus(platform: string): Promise<{ platform: string; connected: boolean; user?: string; workspace?: string }> {
    const resp = await api.get(`/api/platforms/${platform}/status`);
    return resp.data;
  },
  ...
  async platformListDatasets(platform: string, projectId: string): Promise<{ datasets: any[] }> {
    const resp = await api.get(`/api/platforms/${platform}/datasets`, { params: { project_id: projectId } });
    return resp.data;
  },
  ...
  /** Start downloading files from a platform to a processing backend. */
  async startTransferDownload(
    platform: string,
    fileIds: string[],
    targetBackend: string,
    targetPath: string
  ): Promise<{ transfer_id: string; status: string }> {
    const resp = await api.post('/api/transfer/download', {
      platform, file_ids: fileIds, target_backend: targetBackend, target_path: targetPath,
    });
    return resp.data;
  },

  /** Start uploading files from a processing backend to a platform. */
  async startTransferUpload(
    sourceBackend: string,
    sourcePath: string,
    platform: string,
    datasetId: string
  ): Promise<{ transfer_id: string; status: string }> {
    const resp = await api.post('/api/transfer/upload', {
      source_backend: sourceBackend, source_path: sourcePath, platform, dataset_id: datasetId,
    });
    return resp.data;
  },

  /** Transfer data between any two platforms/backends. */
  async startTransferMove(
    sourceType: string,
    sourcePath: string,
    ...
    const resp = await api.post('/api/transfer/move', {
  ...
  async getTransferProgress(transferId: string): Promise<any> {
    const resp = await api.get(`/api/transfer/${transferId}`);
    return resp.data;
  },

  /** Cancel an in-progress transfer. */
  async cancelTransfer(transferId: string): Promise<void> {
    await api.post(`/api/transfer/${transferId}/cancel`);
  },

  /** Get recent transfer history. */
  async getTransferHistory(): Promise<{ transfers: any[] }> {
    const resp = await api.get('/api/transfer/history/list');
    return resp.data;
  },
