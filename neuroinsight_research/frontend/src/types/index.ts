...
export interface Job {
  ...
  /** Source platform if data came from an external platform */
  data_source_platform?: string;

  /** Dataset/experiment ID on the source platform */
  data_source_dataset_id?: string;
}
...
/** Supported data source types (filesystem + external platforms) */
export type DataSourceType = 'local' | 'remote' | 'hpc' | 'pennsieve' | 'xnat';

/** Connection status for an external platform */
...
/** A dataset or project on an external platform */
...
/** A file or directory on an external platform */
...
export interface TransferStatus {
  id: string;
  status: 'pending' | 'downloading' | 'uploading' | 'transferring' | 'completed' | 'failed' | 'cancelled';
  direction: string;
  platform: string;
